...
#ifndef ATOLL_OLED_NUM_PAGES
#define ATOLL_OLED_NUM_PAGES 2
#endif

#ifndef ATOLL_OLED_NUM_FIELDS
#define ATOLL_OLED_NUM_FIELDS 3
#endif

#ifndef ATOLL_OLED_NUM_FEEDBACK
#define ATOLL_OLED_NUM_FEEDBACK 4
#endif
...
class Oled : public Task {
   public:
    ...
    struct OutputField {
        Area area;
        FieldContent content[ATOLL_OLED_NUM_PAGES];
    };
    ...
    OutputField field[ATOLL_OLED_NUM_FIELDS];                 // output fields
    const uint8_t numFields = ATOLL_OLED_NUM_FIELDS;          // helper
    Area feedback[ATOLL_OLED_NUM_FEEDBACK];                   // touch feedback areas
    ...
    const uint8_t numPages = ATOLL_OLED_NUM_PAGES;            // helper
    ...
    virtual void fill(const Area *a,
    ...
    virtual void showTime(bool send = true, bool clear = false) {
    ...
    virtual void onPower(uint16_t value) {
        ...
        int8_t fieldIndex = getFieldIndex(FC_POWER);
        if (-1 == fieldIndex) return;

        // if (!aquireMutex()) return;
        // printfFieldDigits(fieldIndex, false, C_FG, C_BG,
        //                   "%3d", value);
        // Area *a = &field[fieldIndex].area;
        // device->setCursor(a->x + a->w - 20, a->y + a->h);
        // device->setDrawColor(C_XOR);
        // device->setFont(unitFont);
        // device->print("W");
        // device->sendBuffer();
        // releaseMutex();
    ...
    virtual void onCadence(uint16_t value) {
        static uint16_t lastValue = 0;
        if (lastValue == value) return;
        int8_t fieldIndex = getFieldIndex(FC_CADENCE);
        if (-1 == fieldIndex) return;
        printfFieldDigits(fieldIndex, true, C_FG, C_BG,
                          "%3d", value);
        lastValue = value;
    ...
    virtual void onHeartrate(uint16_t value) {
        static uint16_t lastValue = 0;
        if (lastValue == value) return;
        int8_t fieldIndex = getFieldIndex(FC_HEARTRATE);
        if (-1 == fieldIndex) return;
        printfFieldDigits(fieldIndex, true, C_FG, C_BG,
                          "%3d", value);
        lastValue = value;
    ...
    virtual void onSpeed(double value) {
        ...
        if (lastValue == value) return;
        int8_t fieldIndex = getFieldIndex(FC_SPEED);
        if (-1 == fieldIndex) return;
        printfFieldDigits(fieldIndex, true, C_FG, C_BG,
                          "%3d", (uint16_t)value);
        lastValue = value;
    ...
    virtual void onDistance(uint value) {
        ...
        if (lastValue == value) return;
        int8_t fieldIndex = getFieldIndex(FC_DISTANCE);
        if (-1 == fieldIndex) return;
        if (value < 1000)
            printfFieldDigits(fieldIndex, true, C_FG, C_BG,
                              "%3d", value);
    ...
    virtual void onAltGain(uint16_t value) {
        static uint16_t lastValue = 0;
        if (lastValue == value) return;
        int8_t fieldIndex = getFieldIndex(FC_ALTGAIN);
        if (-1 == fieldIndex) return;
        if (value < 1000)
            printfFieldDigits(fieldIndex, true, C_FG, C_BG,
                              "%3d", value);
