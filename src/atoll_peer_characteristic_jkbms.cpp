    ...
    switch (frameType) {
        case 0x01:
            if (protocolVersion == PROTOCOL_VERSION_JK04) {
                log_d("settings protocolVersion 04");
                // decodeJk04Settings(data);
            } else {
                log_d("settings protocolVersion 02");
                // decodeJk02Settings(data);
            }
            break;
        case 0x02:
            if (protocolVersion == PROTOCOL_VERSION_JK04) {
                log_d("cellInfo protocolVersion 04");
                // decodeJk04CellInfo(data);
            } else {
                // log_d("cellInfo protocolVersion 02");
...
/*
    ...
    void decodeJk02Settings(const std::vector<uint8_t>& data) {
        ...
        log_i("  Charge switch: %s", ((bool)data[118]) ? "on" : "off");
        publish_state_(charging_switch_, (bool)data[118]);

        // 122   4   0x01 0x00 0x00 0x00    Discharge switch
        log_i("  Discharge switch: %s", ((bool)data[122]) ? "on" : "off");
        publish_state_(discharging_switch_, (bool)data[122]);

        // 126   4   0x01 0x00 0x00 0x00    Balancer switch
        log_i("  Balancer switch: %s", ((bool)data[126]) ? "on" : "off");
        ...
        publish_state_(disable_temperature_sensors_switch_, check_bit_(data[282], 2));
        publish_state_(display_always_on_switch_, check_bit_(data[282], 16));
    ...
    void decodeDeviceInfo(const std::vector<uint8_t>& data) {
        ...
        log_i("  Serial number: %s", std::string(data.begin() + 86, data.begin() + 86 + 11).c_str());
        log_i("  Passcode: %s", std::string(data.begin() + 97, data.begin() + 97 + 5).c_str());
        log_i("  User data: %s", std::string(data.begin() + 102, data.begin() + 102 + 16).c_str());
        log_i("  Setup passcode: %s", std::string(data.begin() + 118, data.begin() + 118 + 16).c_str());
    }
*/
